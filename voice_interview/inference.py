...
class VoiceInterviewPipeline:
    ...
    def run(
        ...
        # ----------------------------
        # Generate / Reuse Questions
        # ----------------------------
        self.question_engine.start_interview(
            role=role,
            total_questions=5,
            existing_questions_dict=existing_questions_dict
        )
        ...
        while self.manager.has_next_question():
            ...
            print("\n" + "=" * 60)
            print(f"Question {self.manager.current_question}/{len(questions)}")
            print("=" * 60)
            print("\n", question)
            ...
            # Record
            audio_path = self.recorder.record(RECORDING_DURATION)
            ...
            if self.manager.has_next_question():
                ...
                self.tts.speak("Thank you. Here is your next question.")
        ...
        for i, evaluation in enumerate(report["evaluations"], start=1):
            ...
            print()
            print("Better Answer:")
            print(evaluation.get("better_answer", ""))
            print()
            print("Follow-up Question:")
            print(evaluation.get("follow_up_question", ""))

        print("\n" + "=" * 60)
        self.tts.speak("Congratulations. Your interview has been completed.")
        return report


def main():
    pipeline = VoiceInterviewPipeline()
    report = pipeline.run(role="Machine Learning Engineer")
    print("\n")
